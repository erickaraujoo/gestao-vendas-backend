...
export const refreshBlingToken = async ({ grantType, refreshToken, clientAPP }: Input): Output => {
  const data = await fetch(`${blingConfig.URL}/oauth/token`, {
    body: new URLSearchParams({ grant_type: grantType, refresh_token: refreshToken }),
    headers: {
